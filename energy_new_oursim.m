...
%             end
           ...
           % axis off;
            ...
            axis([0 5000 theta_white-5 theta_black+5]);
...
%             end
          pattern(strip)  
           axis off;
            daspect([1 1 1]);
            legend('A** = 81.7197','A** = 100.2040','A** = 106.6036','A** = 145.9835','A** = 170.8699');
            set(gcf,'Color',[1,1,1]);
            hold off;
            subplot(10,1,[9 10]);
            hold on;
            plot(area_arr_angle(1:countt-1)/strip^2, lca_arr(1:countt-1),'-r', 'LineWidth',2);
            plot(area_arr_angle(1:countt-1)/strip^2, rca_arr(1:countt-1),'-g', 'LineWidth',2);
            plot(area_arr_angle(1:countt-1)/strip^2, ca_fitted(1:countt-1),'--k', 'LineWidth',2);
            ...
            grid on;
            hold off;
%[angle,radius]=kusumatmaja(area(length(area_arr)));
